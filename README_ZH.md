...
- **🔘 分场景隐藏屏下指纹**：可分别控制锁屏（含息屏 AOD）和支付/密码管理器/通行密钥等应用内认证弹窗的指纹图标/光圈，同时保留完整的指纹识别功能。
...
- **⌨️ 小白条快捷操作**：由 SystemUI 接管长按与双击，可直达 Gemini、ChatGPT、默认助手和即圈即搜。识别依赖 SystemUI 抢到触摸所有权，因此在 Rust + Flutter 架构的系统桌面 8.0 抢占小白条时并不稳定。
