...
- **⌨️ Gesture Bar Shortcuts**: Lets SystemUI own long-press and double-tap actions for Gemini, ChatGPT, the default assistant, and Circle to Search. Recognition depends on SystemUI winning pointer ownership, so it is unreliable while the Rust + Flutter Launcher 8 gesture recognizer holds the handle.
