...
## Native payload
`app/src/main/cpp/NativeRules/` builds `libhypertweak_native.so` from `app/CMakeLists.txt`; LSPosed injects it via `app/src/main/resources/META-INF/xposed/native_init.list` and calls the upstream `miui_home_native_hook.cpp` entry. The launcher lifecycle, profile resolvers, AOT state hooks, remap repair, fork owner reset, and LSPlt backend are copied from the Apache-2.0 upstream implementation. `clear_button_rule.cpp` and `folder_columns_rule.cpp` are the feature-specific rules; they resolve their targets through the structural registry (see below) and use upstream's validated Dart image and action-down maintenance paths. Their load callbacks consume the supplied handle and never call `dlopen`. The launcher-side rule switches reach the payload through `NativeRules.applyRuleSwitches` (JNI), not through `NativeRuleConfig`'s shared config file: the launcher is a `platform_app_36` process with an ordinary app uid that is neither the owner nor in the `media_rw` group, and scoped storage refuses it the module's `Android/media` directory (mode 770) and file (mode 660). The switches are `hide_recents_clear`, `opened_folder_columns`, and `contextual_search_long_press` (the launcher long-press Circle to Search gate, which used to arrive over a broadcast the removed back-gesture route sent). `liblsplt.so` comes from the vendored AAR in `app/libs/`. The payload is fail-closed and runs its native launcher hooks only in the upstream HYOS launcher process family.
