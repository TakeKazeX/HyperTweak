...
/**
 * Removes the Super Island notification whitelist, OS4 SystemUI.
 *
 ...
 * through `MiuiSettings.SettingsCloudData`) decide whether a package may show a Super Island
 * notification (the Dynamic-Island-style focus bubble) — 1 = allowed, -1 = not allowed,
 * otherwise the user's per-app `<pkg>_focus` preference. Consumers on this build: the
 * `NotificationProvider` / `NotificationProviderPublic` content-provider queries and the
 * shade-menu Super Island toggle
 ...
 * disabled: the shade-menu Super Island off toggle persists `<pkg>_focus = 0` in the
...
object FocusNotificationWhitelistHooker : StaticHooker() {
    ...
    override fun onHook() {
        ...
        DebugLog.d(TAG, "Super Island notification whitelist removed (user-disabled apps kept off)")
    }
