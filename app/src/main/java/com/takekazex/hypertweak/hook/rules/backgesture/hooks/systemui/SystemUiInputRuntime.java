...
public abstract class SystemUiInputRuntime extends HookRuntimeCore {
    ...
    /**
     * HyperTweak: set once this process has heard from a hooked launcher. While it is false the
     * back gesture takes ownership directly instead of waiting for an accepted-input token that
     * would never arrive. Monotonic, so once the launcher announces itself upstream's
     * arbitration owns every later gesture and the two models can never both claim one.
     */
    protected volatile boolean miuiHomeArbiterSeen;

    protected boolean isHyperOsIndicatorEnabled() {
        return readHyperOsBooleanPreference(
                Preferences.KEY_AOSP_BACK_HYPEROS_INDICATOR, false);
    }

    protected boolean isHyperOsHapticsEnabled() {
        return readHyperOsBooleanPreference(
                Preferences.KEY_AOSP_BACK_HYPEROS_HAPTICS, false);
    }

    protected boolean isHyperOsHapticsEnhancedEnabled() {
        return readHyperOsBooleanPreference(
                Preferences.KEY_AOSP_BACK_HYPEROS_HAPTICS_ENHANCED, false);
    }

    @Override
    protected boolean isHyperOsSlideAnimationEnabled() {
        return readHyperOsBooleanPreference(
                Preferences.KEY_AOSP_BACK_SLIDE_ANIMATION, false);
    }

    /**
     * HyperTweak: upstream lazily resolves its own remote preference group here. The module
     * already synchronises {@link Preferences} across processes, so read through it and keep
     * upstream's fail-closed policy when the backing store is unavailable.
     */
    protected boolean readHyperOsBooleanPreference(String key, boolean defaultValue) {
        try {
            boolean enabled = Preferences.INSTANCE.getBoolean(key, defaultValue);
    ...
    protected final class NativeBackInputMonitor extends InputEventReceiver {
        ...
        protected boolean onNativeDown(MotionEvent event) {
            ...
            // HyperTweak: upstream only ever starts the gesture once the launcher publishes an
            // accepted-input token, so on a launcher with no Java gesture stack the gesture would
            // never start at all. Fall back to the pre-0.8.0 model and take ownership directly,
            // synthesising the token from this same DOWN so every downstream check still applies.
            //
            // Only ever taken when the launcher provably cannot arbitrate. The launcher announces
            // itself lazily, on its first gesture-stub interaction, so "we have not heard from it
            // yet" does NOT mean it is absent: claiming a gesture it is also arbitrating leaves
            // the two sides' ownership identities diverged, which strands the return-home
            // transition. LauncherVersion.mayArbitrate fails safe for exactly that reason.
            if (!miuiHomeArbiterSeen && !LauncherVersion.INSTANCE.getMayArbitrate()) {
                moduleLog(Log.INFO, TAG, "Native SystemUI back candidate without a MiuiHome arbiter"
                        + ", inputModel=systemui-direct-pilfer"
                        + ", eventId=" + downEventId
                        + ", downTime=" + downTime
                        + ", displayId=" + downDisplayId
                        + ", edge=" + activeEdge + ", x=" + downX + ", y=" + downY);
                acceptMiuiHomeInput(new MiuiHomeAcceptedInputToken(
                        downEventId, downTime, downDeviceId, downSource,
                        downDisplayId, activeEdge, systemUiInputArbiterGeneration));
                return false;
            }
        ...
        // HyperTweak: getRunningTasks is deprecated for third-party callers, who get only their
        // own task. This runs inside SystemUI, which holds REAL_GET_TASKS and does get the full
        // list, and no public API replaces it for that case. Scoped to this one method rather
        // than suppressed class-wide, so a genuinely stale call elsewhere still shows up.
    ...
    protected final class SystemUiBackGestureDriver {
        ...
        /**
         * HyperTweak: CrossTaskWallpaperRuntime lives in another package, where Java's protected
         * access does not reach this nested class's fields, so expose the flag it needs.
         */
        ...
        /**
         * HyperTweak: a released session is only cleared when Shell reports the animation
         * finished with a matching identity. A navigation that ends without that callback — seen
         * on device right after an app launch, where the release resolves
         * {@code actualTrigger=false, outcome=post-commit} and no finish ever arrives — leaves
         * {@code activeShellSession} set forever, so every later DOWN is rejected with
         * "Suppressed SystemUI back while Shell is busy" and the gesture appears dead until the
         * next successful navigation. Bound the wait: if the session is still active and Shell is
        ...
        protected void scheduleShellSessionReleaseWatchdog(
            ...
            new Handler(Looper.getMainLooper()).postDelayed(() -> {
                ...
                // Controller state is only readable on its owner thread — every other readiness
                // check in this file marshals there, so the watchdog does too. The completion
                // itself still runs on main, matching the normal finish path.
                try {
                    session.executor.execute(() -> {
                        boolean quiescent = false;
                        try {
                            quiescent = isShellReadyOnOwner(session.controller);
                        } catch (Throwable throwable) {
                            moduleLog(Log.WARN, TAG,
                        ...
                        moduleLog(Log.WARN, TAG, "Completing Shell session from release watchdog"
                                + ", shellSessionId=" + session.id);
                        new Handler(Looper.getMainLooper()).post(
                                () -> completeShellSessionOnMain(session, "releaseWatchdog"));
                    });
                } catch (Throwable throwable) {
                    moduleLog(Log.WARN, TAG, "Failed to schedule Shell release watchdog check"
                            + ", shellSessionId=" + session.id, throwable);
                }
            }, SHELL_RELEASE_WATCHDOG_TIMEOUT_MS);
        }
